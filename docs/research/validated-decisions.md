...
| MeLi OAuth: Authorization Code Flow. Scopes: `read`, `write`, `offline_access`. Access token: 180 días. Requiere refresh. | MeLi Developers — Auth. Scopes NO son granulares (no hay scope separado por recurso). | 2026-04-09 |
| MeLi app credentials (client_id, client_secret): GLOBALES (env vars de plataforma). Tokens de cada tenant: POR TENANT en `tenant_integrations`. | Estándar OAuth 2.0 multi-tenant. Confirmado en MeLi Developers. | 2026-04-09 |

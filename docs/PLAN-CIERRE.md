...
- **3.3 = DIFERIDO al día del lanzamiento** (decisión founder 2026-08-28): el Supabase local (podman, homologado a PRD) ES el ambiente de desarrollo + el nightly corre STG efímero en CI — el dev cloud solo agregaría UAT sin túnel y eso ya funciona vía ngrok (probado con Aveonline STG hoy). Triggers para retomarlo: UAT de lanzamiento con tenants reales necesitando ambiente siempre alcanzable · demo desde cualquier lugar · segundo desarrollador. Al dispararse: [F] crea el proyecto Supabase Free y entrega ref/keys/password; [A] ejecuta `scripts/db/replay_migrations_dev.sh` + `bootstrap_dev_sandbox.py` con `KONVI_SAFE_REFS=<ref>` + re-crea los secretos del Vault del dev (pg_dump no copia la Vault root key).

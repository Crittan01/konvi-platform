...
| ID | Estado | Resolución |
|---|---|---|
| Y3 (webhook MeLi, IP spoofeable) | CORREGIDO (código) + PENDIENTE (infra) | Fail-closed en producción si no hay ancla de IP confiable configurada: `services/api/dependencies/security.py` (`client_ip_trust_configured()`), `services/api/routers/meli_webhook.py` `_verify_meli_origin` → 503 si `APP_ENV=production` sin `TRUSTED_CLIENT_IP_HEADER`/`XFF_TRUSTED_HOPS_FROM_RIGHT`; 5 tests nuevos (`tests/test_meli_webhook_origin.py`). Infra (bloquear origen Render directo + canario T4-01) → §3 ítem 6. Residual documentado: con el header configurado, un request SIN él cae al fallback XFF (evadible si el origen acepta tráfico directo). |
...
4. **[Y6] Supabase Dashboard → Auth**: activar *Leaked password protection* + `minimum_password_length ≥ 12`.
5. **[Y6] Decisión founder**: MFA obligatorio para owner/manager (gate AAL2 ya existe; el enrollment es opcional).
6. **[Y3] Infra**: bloquear tráfico directo al origen Render (Cloudflare Authenticated Origin Pull / firewall) y ejecutar el canario T4-01; sin esto, el fail-closed nuevo solo cubre el caso "sin header confiable configurado".
...
9. **[G31] Tras el deploy**: verificar que nada lee el secreto Vault legacy y ejecutar el `DELETE` comentado al final de la migración.

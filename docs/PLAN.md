...
| ENV-1 | **DEV local segregado: Supabase local (podman) + Studio GUI como target por defecto del desarrollo.** El stack ya existe y replica limpio (db-harness, `scripts/dbharness_up.sh`, Studio en `:54323`); falta que las apps (web + servicios) lo usen por defecto: `.env` / `apps/web/.env.local` → `127.0.0.1:54321`, seeds sintéticos vía `scripts/db/bootstrap_dev_sandbox.py`. Limitaciones honestas: Meta/Wompi no llaman a localhost (replay de payloads), Auth/Realtime sin paridad total cloud. **Complemento del checklist §A #16** (dev cloud se reactiva el día del lanzamiento real) | Un error destructivo en dev local nunca toca datos reales — hoy local corre contra prod (riesgo aceptado `prelaunch`, `docs/infra/environments.md` §0) | S-M | Ninguna (no requiere crear proyecto cloud) |

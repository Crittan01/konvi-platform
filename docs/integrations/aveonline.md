...
1. **Registro en el proveedor** (`POST /api/v1/integrations/aveonline/webhook/configure` [owner]):
   ...
   - **Fallback legacy**: AveCRM `avestock/api/createWebhook.php` con `param1_name="secret"` generado localmente (si el endpoint oficial falla para la cuenta). El response del configure informa `mechanism: custom-webhook | legacy-avestock`.

...
**Estado 2026-08-20: CERRADA (3/3 verificadas E2E en STG).** Resend: key `konvi-stg` (sending_access) con envío real a `delivered@resend.dev` por el path del proyecto; política `RESEND_FROM_EMAIL` por ambiente (STG nunca desde `konvi.co` a direcciones sintéticas; objetivo: subdominio `stg.konvi.co` con la fase S3). Telegram: bot `@konvi_stg_bot` + grupo `Konvi STG Operadores` (chat_id `-5381900925`) con notificación de escalación real entregada — procedimiento canónico en `docs/integrations/telegram.md` (chat_id de grupo SIEMPRE negativo; privacy mode exige `/start@bot` en el grupo). Gemini: proyecto GCP `konvi-stg` separado (cuota/billing por proyecto [DOC]) con auth key restringida — llamada real `gemini-3.1-flash-lite` OK. `certify_stg.sh` 18/18 tras el repliegue.

...
- **Reglas de contenido por archivo (depuración 2026-08-21):** `.env.local` = anclas STG (`localhost`, `APP_ENV=staging`) + secretos de STG (ngrok, keys STG de Resend/Gemini) — NO duplicar tuning con `value:` en render.yaml (se hereda solo; duplicarlo enmascara cambios futuros). `.env.prd-backup` = valores REALES de prod (nada de `localhost` ni ngrok — esas son de STG). **Los secretos compartidos por ambos ambientes (ej. `INTERNAL_SERVICE_SECRET`) deben ser DISTINTOS entre STG y PRD** — si son iguales, el de STG queda comprometido de facto (rotado 2026-08-21 tras detectarse iguales). `PUBLIC_WEBHOOK_URL`: STG = ngrok del api, PRD = `https://konvi-api.onrender.com` (transitorio hasta dominio propio).

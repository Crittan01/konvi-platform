...
export function IndexPendingBanner({ pendingCount }: Props) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-3 rounded-xl border border-amber-500/25 bg-amber-500/5 flex-wrap">
      ...
      {state !== 'loading' && (
        <button
          onClick={handleIndex}
          className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-amber-500/15 text-amber-400 border border-amber-500/30 hover:bg-amber-500/25 transition-colors shrink-0"
        >

...
Diseño (igual que el adapter de mensajería):
  ...
  • El adapter resuelve su credencial per-tenant internamente (patrón OAuth
    ...
    `supabase` para tests. EXCEPCIÓN (P1.5): un caller que YA resolvió una credencial
    válida para una operación adyacente (p.ej. el GET pre-PUT del money-path) puede
    inyectarla vía el parámetro OPCIONAL `access_token` de sync_stock/update_listing,
    evitando una 2ª resolución redundante — ahorra un round-trip DB+Vault en el hot-path
    Y cierra la ventana de fallo que abriría re-resolver (si el token expira/refresh falla
    entre el GET y el PUT). El adapter la trata como OPACA; `access_token=None` → self-resolve.

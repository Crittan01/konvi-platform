...
puente sin duplicar lógica de Wompi/Envia/Aveonline.

Rev. 107 M.3+M.5: soporta multi-provider shipping (Envia/Aveonline).
La selección del provider se hace en `tools/shipping.py::QuoteShippingTool`
leyendo `tenant_shipping_provider_config.active_provider` (single enum,
sin fallback automático — ADR-0019).
"""
...
# ─── Shipping quote — provider Aveonline (M.3 rev. 107) ────────────────────
...
async def quote_shipping_for_cart_aveonline(
    ...
    *,
    ...
    Reusa los helpers legacy de geo + package estimate, pero invoca el
    ...
    Return shape idéntico al adapter Envia para preservar el contrato del
    LLM y de los invariantes Python.
    """
    from tools.shipping_quote_tool import (
        _resolve_destination_from_query,
        _get_tenant_shipping_origin,
    ...
    # Construir origin/destination en formato Aveonline.
    # Aveonline acepta `BOGOTA(CUNDINAMARCA)` uppercase o DANE 8-dig (§10.3).
    # Probamos primero el formato nombre (más robusto en DEMO).
    origin_aveonline_city = _to_aveonline_city_format(
        origin.get("city") or "", origin.get("state") or "",
    )
    dest_aveonline_city = _to_aveonline_city_format(
    ...
    try:
        ...
        return {
            "ok": False,
            "error": f"Error inesperado cotizando con Aveonline.",
            "code": "AVEONLINE_UNEXPECTED",
        }

    # Mapear QuoteOption[] → dict shape que matchea el adapter Envia.
    options: list[dict] = []
    ...
    # Persistir quoted_options en DB (mismo patrón Envia — DB-first
    # Plan A.0.2, cross-path / cross-turn).
    try:
        from tools.cart_tool import get_cart_with_items, set_quoted_options
        cart_row = get_cart_with_items(
            supabase, conversation_id=conversation_id, tenant_id=tenant_id,

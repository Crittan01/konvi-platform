"""Coupon engine — Sem 6 I.2 (rev. 105). Hogar canónico: `konvi_domain` (Track 5 M2.0).

ADR: docs/adr/0015-coupon-engine.md

Helpers puros para aplicar/revocar/consumir cupones. Channel-agnóstico
(WhatsApp + futuro storefront web reusan). **Única fuente de verdad**: los
módulos `services/api/lib/coupons.py` y `services/ai-orchestrator/lib/coupons.py`
son shims que re-exportan desde aquí (M2.0 — antes: copia + sys.path hack).

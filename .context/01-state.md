...
**Última actualización**: 2026-08-25 (**Track 7 EN CURSO — T7.1+T7.10+T7.5+T7.2+T7.3+T7.4+T7.6 CERRADOS**: escena auth ×4 + logout de marca + topbar identidad + motion chat inbox dedupe-proof + pill bottom-nav viajera + LayoutItem pedidos + micro-celebraciones de dinero (dinero sandbox real) + barrido de estados — `RouteError` compartido con 23 boundaries por ruta, ~20 vacíos a `EmptyState`, skeletons a `ui/skeleton`; fixes DS de la sonda live: `useReducedMotionDS` + `itemVariantsReduced` + Pressable tabindex SSR. Directiva founder 2026-08-25: la firma impregna TODO el front · antes: **M2.4 CERRADO — M2 COMPLETO**)

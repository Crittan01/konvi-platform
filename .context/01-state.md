...
**Última actualización**: 2026-08-25 (**M2.3 CERRADO** — payment link colapsado: política reuso/TTL única en `konvi_domain.orders.payments` con puertos inyectados + router = adaptador puro + paridad de política bot↔paquete alarmada + reuso live STG verificado vía REST (mismo checkout_url, 1 fila payments) · antes: **M2.2 CERRADO** — cancelación unificada con pipeline legal completo en consola)
...
**Tests**: 4.776 pytest (316 dbharness) + 363 vitest · ruff ≤197 · CI 5/5 · `certify_stg.sh` 18/18 · validate --ci 25/25.

...
**Última actualización**: 2026-08-22 (**PRD DESCONGELADO y desplegado** — Track 9 cerrado en STG + 8 migraciones aplicadas a prod por protocolo + deploy `eec5534f` verificado · auditoría profunda del bot + B-0 + conformidad Aveonline + E2E STG)
**Branch activo**: `develop`; `production` = `eec5534f` (deploy 2026-08-22, los 4 servicios live verificados vía Render API + health ×5).
**Ledger**: 262 migraciones = repo = prod (interlock migrate-before-deploy respetado: migraciones aplicadas ANTES del deploy).
...
2. **Track 6 — Alineación total con docs oficiales — EN CURSO (4/6 cerradas)**, estado exacto abajo §"Track 6 — estado por tecnología (continuación)".
...
| **Resend** | 🔶 **A MEDIAS — continuar aquí** | sin commitear (working tree) | Hecho: senders con tags (tenant_id/order_id/template/event_type), User-Agent explícito, text/plain siempre, reply_to al tenant, log de cuota x-resend-*; tests ajustados (`test_rev94`, `test_rev100`, `test_resend_payload`, `test_wompi_webhook_money_paths` — 189 passed en el filtro). **FALTA (diseño ya en matriz del dossier de investigación):** (1) migración tabla `email_events(id uuid pk, svix_id text unique, tenant_id uuid, order_id uuid, email_id text, event_type text, recipient text, payload jsonb, occurred_at, received_at)` + RLS por tenant + REVOKE a clientes (patrón Track 9); (2) router `services/api/routers/resend_webhook.py` → `/api/v1/webhooks/resend` registrado en `main.py:332-341`, verificación firma svix (lib `svix` Python sobre raw body, secret `RESEND_WEBHOOK_SECRET` sync:false en render.yaml + .env.example + contrato), 200 rápido + BackgroundTask, dedup por svix_id (ON CONFLICT DO NOTHING), routing tenant/order desde tags; (3) en bounced/complained/failed/suppressed → alerta Telegram operadores (path `notify_escalation_async`); suppression.added → excluir dirección en senders; (4) tests estilo `test_rev94` (firma válida/inválida, dedup, routing); (5) E2E STG con `bounced+test@resend.dev`/`complained+test@resend.dev`; (6) doc maestro `docs/integrations/resend.md` (hoy el README dice "sin doc maestro") + registrar webhook en dashboard Resend STG (1 endpoint gratis) |
| **Telegram** | ⬜ PENDIENTE (diseño en matriz) | — | (1) `parse_mode` legacy → HTML en 3 senders (`services/ai-orchestrator/notifications.py:108`, `services/api/lib/operator_alerts.py:76`, `services/api/routers/telegram_webhook.py:356`) con `html.escape` — elimina el fallback "can't parse entities"; (2) inline keyboard en alerta de takeover + handler `callback_query` en telegram_webhook (hoy se descarta en silencio — la UI ya lo promete) + `answerCallbackQuery` (obligatorio) + `editMessageReplyMarkup` anti doble-click (requiere persistir `message_id` de alertas); (3) endpoint `POST /api/v1/integrations/telegram/setup` (getMe→setWebhook con allowed_updates=["message","callback_query"]→setMyCommands→getWebhookInfo) — cierra M17 (setWebhook manual); (4) puntos de extensión documentados: mensajes efímeros (Bot API 10.2 — PII de /estado solo al que pregunta), message_thread_id para grupos-foro, secret per-tenant |
| **MeLi** | ⬜ Condicionado a S6 | — | El plan la difiere a cuando el marketplace opere (S6: app de prueba MeLi). Documentar como tal |

**Cierre de Track 6 (tras Resend+Telegram):** suite completa + dbharness + certify_stg 18/18 + validate --ci + push + CI verde por tecnología + bitácora PLAN.md §E por tecnología + migraciones Track 6 nuevas a PRD por protocolo en el próximo deploy (`20260822130000` Meta enums + `20260822130100` Gemini columns + la de email_events si existe) — con su SELECT pre-vuelo si aplica. **Nota de entorno:** Supabase CLI pineada 2.90.0 (NO actualizar — la 2.115.0 rompe homologación STG↔PRD, ver HANDOFF §6); tras cada replay (`schema_drift_check.sh --update`) restaurar STG con `scratch/track9_filter_dump.py` + `scratch/track9_restore_stg.sh` + `scratch/track9_restore_stg.py` (backup de secretos ANTES, destruirlo DESPUÉS).

...
**Última actualización**: 2026-08-29 (**CIERRE AUDITORÍA SEGURIDAD OWASP 2026-08-23** — 30 hallazgos procesados: 24 corregidos en repo (web 9, Python 14, secretos/docs, migración DB `20260828120000` aplicada en STG local), 3 riesgos aceptados con evidencia (G21/G23/G31-defensiva), 2 parciales justificados (Y5 sin security_invoker — regresión funcional; Y12 tenant-media público — frontend usa URLs públicas), externos pendientes del founder: **rotar Meta App Secrets ×2 + verify token HOY** (commit huérfano `be739a4` sigue público, HTTP 200), purge GitHub Support, toggles Auth dashboard — documento de cierre con checklist: `docs/auditoria_seguridad_cierre_2026-08-29.md` · gates: validate 18/18, pytest ~4865, dbharness 316/316, vitest 447/447, pip-audit/osv limpios). Anterior: 2026-08-28 (**B-2 FASE 0 + FASE 1 + H11 CERRADOS — y DEPLOY PRD `d8684b97` live** (Fase 0 + H11, código puro; 3 servicios que no auto-dispararon fueron desplegados vía Render API sobre el mismo commit, health ×5 verde). Fase 1: normalizadores de inbound extraídos (`inbound_normalizers.py`) · TurnContext a dos tiempos naciendo en `dispatch_message` (gates comparten la lectura de la conversación; opt-out por return del handler — P11) · filtro de dominio antes de los resolvers de dinero · **TurnFinalizer único** (`turn_finalizer.py`) para la cola del turno (trace/race-gate/envío/escalaciones/audit/resumen) · degraded path deduplicado (P13) · dispatcher.py 3909→3631 LOC · suite agentic 819 · validate 25/25 · harness: money/s19/s17/t8r ✅ (full run con falla única = cuota free-tier Gemini agotada — provider-side probado en log; red limpia = nightly) · commit `aaab646b` · SIGUE: **B-2 Fase 2** — embudo consolidado + handlers sin dinero (consent/PII_COLLECTION, recipient, routing tools/guardrails unificado, COD unificado en ctx) · antes: **B-2 FASE 0 + H11 CERRADOS** — formulación `docs/architecture/bot-dispatcher-reengineering.md` VALIDADA por founder 6/6 §4 (premisa: calidad/efectividad sin importar esfuerzo) · Fase 0: TurnContext v0 + StateResolver al INICIO del turno + V2 lazy + transitions.py cableada (telemetría UNEXPECTED) + 10 constantes muertas fuera — sin cambio de comportamiento · H11 (reclamo→consent-de-compra en loop, founder live PRD): voz LLM (reglas de reclamo en prompts por-estado) + acción determinística (FakeEscalation con enclíticos/markdown — takeover real siempre) + consent framing-reclamo en el embudo + D4 (🙌 fuera) — xfail `t8_reclamo_coherente` RETIRADO · harness: s19 ✅ money_full_flow ✅ · validate 25/25 (pytest 4847 + vitest 435) · commits `fc4f72e6`/`56a52bc8`/`b9a482d7`/`6f41b992`. Anterior: **BLOQUE BOT INICIADO** — paso 6 ejecutado: inventarios INV-A (70% política outbound fuera del embudo, 5 canales lo saltan) + INV-B (34 bloques/turno, lecturas DB duplicadas medidas, 15 parches) en `.audit/findings/2026-08-28-bot-*.md`. Deploy PRD `d82d714c` (2026-08-27): 7 migraciones por protocolo (ledger 269=269) + M2 + Track 7 + Track 3 + fix Aveonline RS256 — y 2026-08-28 el founder repitió el click "Configurar webhook": **201 Created + mechanism=custom-webhook + token oficial — bug Aveonline cerrado de verdad**. Founder [F] pendientes: runbook consolidado `docs/operations/runbooks/founder-console-steps.md`. Antes: **TRACK 7 COMPLETO** + **TRACK 3 EJECUTADO** (autorización founder 2026-08-27): **3.4 ✅** media privada aplicada en PRD (1 objeto, signed URL 200, pública vieja cerrada) · **3.2 ✅** `PYTHON_VERSION=3.13.15` ×3 servicios Python live (gate CI `py-compat-313` creado y verde — la "compat ya verificada" previa era falsa) · **3.1(a) ✅ AMBOS dominios LIVE** (`api.konvi.co/health` 200 + `app.konvi.co/login` 200 — founder agregó los CNAME en Cloudflare DNS-only; verify vía API 202) + **Fase 2 webhooks ejecutada**: `PUBLIC_WEBHOOK_URL` + `NEXT_PUBLIC_WEBHOOK_HOST` → `https://api.konvi.co` (render.yaml + Render API + redeploys live) · por proveedor: Telegram nada que migrar · **Aveonline ✅ migrado DE VERDAD 2026-08-28** (tras el fix RS256 en PRD: 201 Created + `mechanism=custom-webhook aveonline_ok=True` en log + token oficial en DB + endpoint 401-live; el intento de 08-27 a.m. era el fail-silent del bug HS256→RS256 — corregido) · Wompi/Resend con sus dashboards al activarse · Meta queda en el connector onrender (decisión documentada) · **`connector.konvi.co` mapeado como pendiente** (3er dominio $0.25/mes; orchestrator NO necesita dominio — superficie interna verificada) · 3.1(b) protection DIFERIDA (gateada a plan Pro — medido por founder en Dashboard 2026-08-28; con workspace de 1 solo admin no protege contra nadie: trigger = primer colaborador en el workspace), isolation diferido por decisión documentada · 3.3 = proyecto Supabase dev [F] → replay lo hace [A] · **harness nocturno B-3 operativo** (secrets GH registrados + run 33135919388 verde: 22/0/7 xfail) — antes: **T7.12: `PageHeader` transversal a TODOS los módulos** — 25 headers estándar + home (saludo con text-gradient, `title` ReactNode) + inbox (mini-tile inline, excepción UX-UI §5) + detalles pedido/comprobante (`print:hidden` — impreso byte-idéntico) + visor legal; bug pre-existente cazado por la sonda: `legal/view/[doc]` era `force-static` bajo layout autenticado → 307 a /login, ahora `force-dynamic` — antes T7.1+T7.10+T7.5+T7.2+T7.3+T7.4+T7.6+T7.7+T7.8+T7.11: escena auth ×4 + logout de marca + topbar identidad + motion chat inbox dedupe-proof + pill bottom-nav viajera + LayoutItem pedidos + micro-celebraciones de dinero (dinero sandbox real) + barrido de estados — `RouteError` compartido con 23 boundaries por ruta, ~20 vacíos a `EmptyState`, skeletons a `ui/skeleton` + virtualización medida: NO paga (dep `react-virtual` retirada, UX-UI §4.6) + cobertura de render del DS (20 tests, gap §6.5 cerrado) + **settings/security con la firma: `PageHeader` NUEVO en el DS (tile degradado primary→amber + glow + glifo blanco + stagger) — piloto del patrón que T7.12 hará transversal**, alerts de security a variantes semánticas. Fixes DS de las sondas live: `useReducedMotionDS` + `itemVariantsReduced` + Pressable tabindex SSR + fix flake jsdom (`afterEach(cleanup)` ×6). Directiva founder 2026-08-25: la firma impregna TODO el front · antes: **M2.4 CERRADO — M2 COMPLETO**)
**Branch activo**: `develop`; `production` = **`199f4108` (deploy 2026-08-29 — cierre auditoría OWASP 2026-08-23: 72 archivos, 30 hallazgos procesados + locks con hashes + migración `20260828120000` ya aplicada a PRD).** Push develop→production FF → autoDeploy ×4 live en ~2.5 min · post-deploy verificado: health ×5 200 · api reporta version `199f4108` · OpenAPI 404 ×3 servicios (G16 live) · /status 401 sin secret (G17 live) · MeLi webhook 403 allowlist intacto con `TRUSTED_CLIENT_IP_HEADER=cf-connecting-ip` ya seteada (Y3 fail-closed no rompe). Anterior: `d8684b97` (deploy 2026-08-28 — B-2 Fase 0 + H11, código puro).
**Ledger**: **270 migraciones repo = 270 ledger prod** — `20260828120000_owasp_20260823_db_fixes.sql` aplicada a PRD 2026-08-29 con protocolo smoke(BEGIN/ROLLBACK)→apply→repair + verificación post completa (0 funciones sin search_path · anon solo INSERT audit_log · payments_safe solo SELECT · 0 DML authenticated en 9 service-only · GREEN-31 copiado · 7 comments · health ×5 200). Nota de protocolo: el smoke detectó que INSERT directo a `vault.secrets` falla en cloud (42501 `_crypto_aead_det_*`) — la vía soportada es `vault.create_secret` (así quedó la migración).
**Tests**: ~4.865 pytest (316 dbharness) + 447 vitest · ruff ≤196 · CI 5/5 · `certify_stg.sh` 18/18 · validate 18/18 (default) — +5 tests nuevos meli fail-closed, +2 proxy fail-open, +2 inyección phone (cierre OWASP 2026-08-29).
...
**En curso / sigue (en este orden — REORDEN founder 2026-08-24: plataforma primero, BLOQUE BOT AL FINAL):**
1. ~~Track 9 — SEGURIDAD DB~~ ✅ CERRADO y aplicado a PRD 2026-08-22 (bitácora PLAN.md §E).
2. ~~Track 6 — Alineación total con docs oficiales~~ ✅ CERRADO 2026-08-22 (6/6) — bitácora PLAN.md §E.
3. ~~B-1 calidad conversacional~~ ✅ CERRADA 2026-08-22 (bitácora PLAN.md §E).
4. ~~**B-3 harness de evaluación serio (incluye Track 8 adversarial)**~~ ✅ CERRADO 2026-08-23/24 (bitácora PLAN.md §E): assertions de outcome en DB obligatorias (runner rechaza escenarios sin ellas), aislamiento fail-closed (el reset verifica DB y ABORTA el run), 28 escenarios + 10 adversariales t8_*, CI nocturno (workflow `harness-nightly.yml` — requiere 4 secrets GH [F]), corpus dorado (PRD 4d608efd anonimizada + E2E real) + LLM-judge + métricas SQL sobre agentic_shadow_log. **Es la red que hace seguro el reorden: certifica cada noche que el bot actual sigue verde mientras la plataforma evoluciona debajo.**
5. **EN CURSO — la plataforma primero**: Track 5 (M1-M5 dominios modulares) → Track 7 (UX/UI consola) → Track 3 (infra PRD) → remanentes Track 1/2 [F] → Track 4 ops. Orden completo y razón: `docs/PLAN-CIERRE.md` §Orden. **Track 5: M1 ✅ CERRADO 2026-08-24** (inventario verificable 11 dominios → `docs/architecture/domain-capabilities-inventory.md` — backlog exacto de la capa de servicios; drift medido: cancelación ×2, cliente Aveonline ×2, restock ×3, ingreso reconocido ×5, consent ×4) **+ M2 DISEÑO APROBADO por founder 2026-08-25 (4/4 recomendadas: shared-py · pipeline cancelación completo en consola · reason cerrado+detalle · lecturas consola→REST)** → `docs/architecture/domain-services-contract.md` §8 **+ M2.0 ✅ CERRADO 2026-08-25** (paquete `packages/shared-py/` `konvi_domain` 0.1.0: motor de cupones ADR-0015 como única fuente — shims en ambos servicios, sys.path hack muerto — + primitivas `Actor`/`DomainError`/`DomainEvent`; wiring Render/CI/nightly/Makefile verificado empíricamente; guard `tests/test_konvi_domain_pact.py`; suite 4724 + dbharness 316 + certify_stg 18/18 + validate --ci 24/24) **+ M2.1 ✅ CERRADO 2026-08-25** (`konvi_domain.orders`: create/get/list/list_by_contact extraídos intactos — router = adaptador (dual-auth/RBAC/MFA/Idempotency-Key/audit + mapeo DomainError→HTTP) · FSM como fuente única del paquete · **`GET /api/v1/orders/` NUEVO** (hueco M1 §H2) · consola orders migra su listado a REST · `ORDERS_CONTRACT` declarativo (8 operaciones, 4 implementadas) · 25 tests nuevos (unit servicio + estructural contrato + paridad canal↔canal + pact) · **verificación live STG**: list con conteos + 422/401 + create total $110.000 exacto + replay Idempotency-Key sin duplicado · suite 4736 + dbharness 316 + certify_stg 18/18 + validate --ci 24/24). El efecto de stock al confirmar se inyecta (`on_confirm_stock`) — sigue en el API acoplado a `sync_meli_stock` hasta InventoryService (M1 #3). **M2.2 ✅ CERRADO 2026-08-25** — cancelación unificada: pipeline legal completo extraído intacto a `konvi_domain.orders.cancellation` con **puertos inyectados** (`CancellationPorts`) · consola cancela vía PATCH (una sola superficie, sin endpoint nuevo) con el pipeline completo (triage registrado, audit SIC `order_cancellations`, void Wompi en ventana, cancel guía Aveonline, restock idempotente reservas+movements, WhatsApp cliente + Telegram operador) · regla de canal: la escalación bloquea solo a `customer` (staff procede y registra la señal) · **paridad de outcome bot↔paquete certificada** (`test_cancellation_outcome_parity.py`, 11 escenarios, misma huella DB — la duplicación time-boxed del bot tiene alarma) · adaptador con 7 tests (`test_orders_cancel_adapter.py`) · **bug destapado por la certificación LIVE STG**: actor de consola debe ser `operator` (enum `order_cancellation_actor` — "owner" rompe con 22P02; fijado + documentado) · hook `on_stock_restored` preserva el sync MeLi que la consola ya tenía · gate ruff extendido: `packages/shared-py/` con **cero deuda permitida** (sin baseline) · live STG: ciclo completo create+confirm (stock 18→16) → cancel (stock vuelve a 18, ledger sale+cancellation_refund sin duplicar, audit completed) · harness B-3 `money_full_flow` + `s11_cancela_preconfirmacion` PASARON (bot verde). **M2.3 ✅ CERRADO 2026-08-25** — payment link colapsado: política reuso/TTL única en `konvi_domain.orders.payments` (mata el espejo router↔bot M1 §3.3) con puertos inyectados (lazy import en call time) · `DomainError.http_status` opcional (503 "Wompi no configurada") · router = adaptador puro · shim `integrations/wompi_client` re-exporta el TTL · paridad de política bot↔paquete alarmada (`test_payment_link_policy_parity.py`) · 21 tests nuevos (15 unit + 6 paridad) + los 13 del endpoint intactos · lección xdist nueva (binding huérfano por purga de `integrations.*` → factories compartidas a `tests/helpers/`) · live STG: reuso vía REST (mismo checkout_url, 1 fila payments, orden pending_payment) + harness money_full_flow/s11 verdes · suite 4776 + dbharness 316 + validate --ci 25/25 + certify 18/18. **M2.4 ✅ CERRADO 2026-08-25 — M2 COMPLETO (pilotos pedidos+reclamos)**: `konvi_domain.claims` 7/7 ops — UN writer (reason cerrado + `reason_detail` libre opcional + dedup heredada del bot + titularidad por actor + unión de eventos audit/claim_audit/Telegram-puerto) · get (id\|ticket, scoping customer fail-closed) · list con embeds + reason_detail (consola migra su listado a REST, decisión #4) · list_by_contact NUEVO (hueco M1 §3.8) · transition (FSM formalizada: refunded FINAL, refunded_amount write-once sellando KPI, reapertura solo owner) · reversión delegada en RPCs (R2) con `_MOTIVO_HTTP` → `DomainError.http_status` · migración `20260825180000` (ledger 269) · router = adaptador puro JWT-only (RBAC asimétrico G-4 intacto; dedup → 200 `deduplicated:true`) · F-5 movida a `lib/claim_ports.py` · `CLAIMS_CONTRACT` 7/7 implemented · 38 tests nuevos (33 unit + 5 paridad bot↔paquete alarmada — el `status_human` extinto del bot queda como deuda M3 registrada) · live STG vía REST con JWT real (create+dedup+F-5 encolado+reversión end-to-end RV-000003) · suite 4826 + dbharness 316 + validate --ci 25/25 + certify 18/18 + harness money_full_flow ✅ / s19_reclamo ✅ intermitente H11 documentado + bot intacto (diff cero agentic/**) · lecciones nuevas: repair del ledger tras restore track9 con migración nueva + fixture schema canonical regenerado LOCAL (no --linked) para columnas pendientes de deploy. Bitácora completa: PLAN.md §E (2026-08-25, M2.4). **Track 7 COMPLETO 2026-08-27 — cerrados T7.1-T7.12 + T7.9 (opcional ejecutado) + BUG-105-02** · **TRACK 3 EJECUTADO 2026-08-27: 3.4 ✅ + 3.2 ✅ (3.13.15 ×3 servicios live) + 3.1(a) custom domain creado (inerte hasta CNAME [F]) · 3.1(b) protection DIFERIDA (exige plan Pro — medido founder 2026-08-28; workspace 1 admin: trigger = primer colaborador) · 3.3 dev cloud DIFERIDO al día del lanzamiento (decisión founder 2026-08-28: el Supabase local ES el ambiente de desarrollo; trigger = UAT lanzamiento sin ngrok / demo remota / segundo dev)** (escena auth ×4 + logout marca · motion chat dedupe-proof · NavPill + LayoutItem · micro-celebraciones dinero · topbar identidad · barrido estados RouteError+EmptyState+skeletons · virtualización medida: no paga, dep retirada · cobertura render DS ×6 primitivos · settings/security con la firma + `PageHeader` piloto · **T7.12: PageHeader transversal a TODOS los módulos** · **T7.9: `SwipeActions` DS + swipe catálogo móvil + carruseles KPI móviles en catálogo/finanzas** — bitácora PLAN.md §E) **+ BUG-105-02 ✅ cerrado (obsoleto, verificado live: inbox refresca sin F5)**. **Track 3 RESUELTO 2026-08-27/28** (dominios live + Fase 2 webhooks + pin 3.13.15 + G8b + Aveonline de verdad; protection + dev cloud diferidos con triggers documentados — PLAN-CIERRE §Track 3) → **BLOQUE BOT INICIADO 2026-08-28** (ítem 6 abajo). M3 queda dentro del BLOQUE BOT al final (tools del bot generadas del contrato).
6. **AL FINAL — BLOQUE BOT (todo el bot: núcleo, GUI, API, métricas)**: **INICIADO 2026-08-28** — paso 6 (entrada) en ejecución: inventario de inclusiones parche corriendo (Brief A: manipulación outbound fuera del embudo OutputValidator · Brief B: radiografía resolvers pre-LLM para B-2 — `.audit/findings/2026-08-28-bot-*.md`) → su ensamble = formulación arquitectónica que el founder valida → B-2 re-ingeniería del dispatcher sobre contratos estables (resuelve xfails H1-H8 del harness) → B-4 observabilidad/métricas + decisión canary con telemetría acumulada → bot GUI/API en consola (Track 7 ya cerrado). **B-2 FASE 0 + H11 ✅ CERRADOS 2026-08-28** (validación founder 6/6 §4 con premisa calidad-sin-importar-esfuerzo): TurnContext v0 (una lectura por entidad/turno + cart refrescable + derivados del history) · StateResolver al INICIO del turno (post-router; bypass conservan re-resolución post-hoc para badge) · V2 lazy · transitions.py cableada (UNEXPECTED→WARNING) · 10 constantes muertas fuera · **H11** (reclamo→consent-compra en loop, founder live PRD): voz LLM (reglas de reclamo en prompts por-estado) + acción determinística (FakeEscalation enclíticos/markdown) + consent framing-reclamo en embudo + D4 (🙌 fuera) — xfail `t8_reclamo_coherente` retirado · harness s19/money_full_flow ✅ · validate 25/25 · commits `fc4f72e6`/`56a52bc8`/`b9a482d7`/`6f41b992` · **SIGUE: B-2 Fase 2** (embudo consolidado + handlers sin dinero: consent/PII_COLLECTION, recipient, routing tools/guardrails unificado, detección COD unificada en ctx). **B-2 FASE 1 ✅ CERRADA 2026-08-28** (commit `aaab646b`): normalizadores de inbound extraídos (`inbound_normalizers.py`) · TurnContext a dos tiempos naciendo en `dispatch_message` (`for_gates` + `ensure_core` — gates comparten la lectura de la conversación; opt-out por return del handler — P11) · filtro de dominio antes de los resolvers de dinero · **TurnFinalizer único** (`turn_finalizer.py`: trace + race-gate + envío embudo + escalaciones BLOCK/silent + audit + summary-regen) · degraded path deduplicado (P13 — `_escalate_conversation_to_human` con source/severity; el helper ahora notifica aunque falle el update de status) · dispatcher.py 3909→3631 LOC · suite agentic 819 + tests nuevos 10 · validate 25/25 · harness clave ✅ (money/s19/s17/t8r) · full run con falla única = cuota free-tier Gemini agotada (provider-side probado; red limpia = nightly).
7. **PRD descongelado 2026-08-22** (deploy `eec5534f` verificado). **Smoke de dinero real (pago/guía reales PRD) APLAZADO AL FINAL por directiva founder** — la certificación de dinero la hace el harness turno a turno en STG (`money_full_flow`).
8. Pendientes founder registrados [F] — **runbook consolidado con pasos exactos + fuentes oficiales + mi validación posterior: `docs/operations/runbooks/founder-console-steps.md`** (2026-08-28): deadline Meta 2026-09-30 (método de pago por WABA) · WABA hygiene (desuscribir app prod "KAIU Chat" 2024793711712790 de la WABA de prueba — medido vía subscribed_apps) · M19 (verify_token dev, runbook rotation §2.11) · anular guía UAT 86732771636 (panel Aveonline — sin API, CAN-1) · Resend webhook (dashboard + RESEND_WEBHOOK_SECRET) · Wompi prod keys · S6 MeLi · B3/B6 legal · A1 MFA cuando decida · ~~4 secrets GH para el CI nocturno~~ **✅ REGISTRADOS 2026-08-27 + nightly VERDE verificado** (run 33135919388: **22 pasaron · 0 fallaron · 7 xfail** deuda conocida H1-H8, `money_full_flow` verde, ~15 min — la red de seguridad del reorden vuelve a operar cada noche 02:23 COL). **Track 3 RESUELTO 2026-08-27/28** (detalle §5 y PLAN-CIERRE §Track 3): dominios live + Fase 2 webhooks + pin 3.13.15 ×3 + G8b aplicado + Aveonline migrado de verdad (fix RS256) · protection + dev cloud diferidos con triggers documentados.

### Track 6 — estado por tecnología (continuación detallada)

Patrón por tecnología: matriz capacidad × doc oficial vigente (fetch live, URL citada) × estado en código → adoptar ahora (con tests + certificación STG) / punto de extensión documentado → dossier en `docs/integrations/<tec>.md` → commit → CI verde. **Las 6 matrices de investigación ya están ejecutadas** (fetch live 2026-08-22) y resumidas en cada dossier.

| Tecnología | Estado | Commit | Qué se hizo |
|---|---|---|---|
| **Wompi** | ✅ CERRADA | `17605204` | 4 llaves capturables (pub/integrity opcionales, merge no-destructivo, validación prefijos S0.2 extendida + 5 tests) · copies UI corregidos (SHA256 no HMAC; eventos token=suscripciones) · dossier con matriz · datos prueba sandbox para UAT B4. **Pendiente decisión founder:** `taxes` (IVA desglosado en checkout — amarrado a B6/DIAN) |
| **Meta WhatsApp** | ✅ CERRADA | `03ad065f` | Enums template HSM vigentes (+7 estados, CHECK constraint migración `20260822130000`, SENDABLE={APPROVED,REINSTATED,UNARCHIVED}) · health check sin campo deprecado · webhook fields nuevos con persistencia (template_category_update→category; user_preferences stop→consent_comercial_revoked_at, resume NO muta; account_alerts→tenant_provider_health) · mark-read+typing al reclamar inbound (worker) · deadline OPS 2026-09-30 documentado |
| **Supabase** | ✅ CERRADA | `e7bb52e7` | Realtime select de columnas + anti-truncamiento (re-fetch por id) en hooks inbox · fallback legacy ANON_KEY eliminado · runbook DR vault root key en HANDOFF §6 · dossier nuevo `supabase.md` · test cross-tenant payments_safe. **Pendientes ops [F]:** secret key por servicio (4 sb_secret_) · cerrar ciclo JWT signing keys (revocar legacy tras 75min) |
| **Gemini** | ✅ CERRADA | `80c980a9` | Telemetría breakdown (prompt/cached/thoughts → agentic_shadow_log, migración `20260822130100`) · SDK 2.11→2.19 (suite 4568 verde) · VALIDATED tras flag `AGENTIC_TOOL_VALIDATED_ENABLED` (canary STG) · correcciones doc-código (EOL, precios, embeddings) · dossier `gemini.md`. **Decisión documentada:** prefijo del prompt NO reordenado sin evidencia de caching ni harness B-3 |
| **Resend** | ✅ CERRADA | `e03b46d5` senders + `3d41521c` webhook | Senders con tags/User-Agent/text/reply_to/cuota (e03b46d5) · migración `20260822130200` tabla `email_events` (svix_id UNIQUE, RLS tenant, REVOKE Track 9) · router `resend_webhook.py` (firma svix lib oficial, 200 rápido + BackgroundTask, dedup, routing por tags, correlación suppression vía source_id, alerta Telegram en bounced/complained/failed/suppressed) · suppression list local excluye direcciones en TODOS los senders · doc maestro `resend.md` · E2E STG: firma válida/inválida/dedup/suppression verificados live + alerta real al grupo (registro del webhook en dashboard Resend = **[F]**, la key STG es sending_access — 401 restricted_api_key verificado) |
| **Telegram** | ✅ CERRADA | `56f3e31e` | parse_mode HTML con html.escape en los 3 senders (fallback "can't parse entities" eliminado) · inline keyboard "✅ Resolver" en alerta takeover + handler callback_query + answerCallbackQuery + editMessageReplyMarkup (tabla `telegram_alert_messages`, migración `20260822130300`) con cierre cross-canal (callback/comando/consola) · endpoint `POST /telegram/setup` (getMe→setWebhook→setMyCommands→getWebhookInfo) **cierra M17** — verificado live contra `@konvi_stg_bot` · puntos de extensión documentados (efímeros 10.2, threads foro, secret per-tenant) |
| **MeLi** | ⬜ Condicionado a S6 | — | El plan la difiere a cuando el marketplace opere (S6: app de prueba MeLi). Documentar como tal |

**Cierre de Track 6 (ejecutado 2026-08-22):** suite 4906 + dbharness 316 + certify_stg 18/18 + validate --ci + commits por tecnología + CI verde + bitácora PLAN.md §E. Las 4 migraciones Track 6 (`20260822130000` Meta enums · `20260822130100` Gemini columns · `20260822130200` Resend email_events · `20260822130300` Telegram alert_messages) van a PRD por protocolo en el próximo deploy autorizado — con su SELECT pre-vuelo si aplica. **Nota de entorno:** Supabase CLI pineada 2.90.0 (NO actualizar — la 2.115.0 rompe homologación STG↔PRD, ver HANDOFF §6); tras cada replay (`schema_drift_check.sh --update`) restaurar STG con `scratch/track9_backup_secrets.py` (BACKUP CORRECTO: leer `decrypted_secret`, NO `secret` — la columna cruda es ciphertext y doble-cifra en silencio; incidente 2026-08-22 recuperado desde el dump completo pre-reset con id+nonce) + `scratch/track9_restore_stg.sh` + `scratch/track9_restore_stg.py` (ahora con verificación por forma + re-sync de secuencias serial) — backup de secretos ANTES, destruirlo DESPUÉS. **Pendientes [F] registrados:** registrar el webhook de Resend en el dashboard (STG y PRD — la key STG es sending_access y no gestiona webhooks: 401 restricted_api_key verificado) y setear `RESEND_WEBHOOK_SECRET` en Render.
...
| ID | Acción | Razón |
|---|---|---|
| H7 | Rotar Supabase service_role + anon_key + DB password + Meta secret + Wompi sandbox keys | Commit `be739a4` (2026-04-06) tenía `.env` con credenciales reales del proyecto productivo `***SUPABASE_PROJECT_REF_REDACTED***`; ya removido en commit 488c6c6 pero la historia git pushed a GitHub conserva el plaintext. **2026-08-29 (cierre auditoría OWASP):** legacy anon/service_role DESACTIVADAS (2026-08-19) y DB password reseteada ✅ · el commit huérfano SIGUE servido por GitHub (HTTP 200 verificado) · **PENDIENTE founder: rotar Meta App Secrets ×2 + verify token y confirmar lote Wompi/Telegram/MeLi/Resend/Aveonline** — checklist §3 de `docs/auditoria_seguridad_cierre_2026-08-29.md`. |
| H8 | (Opcional) git history rewrite con `git filter-repo --path .env --invert-paths` | Destructivo (cambia hashes de TODOS los commits); requiere coordinación con cualquier dev con clones locales. **2026-08-29: DECIDIDO — ni rewrite ni purge vía Support.** El lote `.env` quedó 100% rotado/validado (sonda activa 2026-08-29) → el huérfano solo expone valores muertos; cierre definitivo = eliminar y recrear el repo de GitHub al final (checklist de recableo en §3-3 del cierre OWASP). |
...
- `scripts/wipe_conversation.py`: vacía la conversación de un teléfono (default `+57312XXXXXX649`).

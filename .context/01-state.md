...
6. **AL FINAL — BLOQUE BOT (todo el bot: núcleo, GUI, API, métricas)**: entrada = inventario de inclusiones parche (brief preservado en `.audit/findings/2026-08-23-patch-inventory-brief.md`) → B-2 re-ingeniería del dispatcher sobre contratos estables (resuelve xfails H1-H8 del harness) → B-4 observabilidad/métricas + decisión canary con telemetría acumulada → bot GUI/API en consola (Track 7 ya cerrado).

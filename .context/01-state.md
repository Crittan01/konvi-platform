...
**Última actualización**: 2026-08-25 (**M2.4 CERRADO — M2 COMPLETO (pilotos pedidos+reclamos)**: ClaimsService 7 ops en `konvi_domain.claims` — UN writer (reason cerrado + `reason_detail` + dedup + titularidad por actor + unión de eventos) · FSM formalizada (refunded FINAL write-once) · reversión delegada RPC · consola claims sobre REST con dialog de detalle · paridad bot↔paquete alarmada · live STG vía REST con JWT real (create+dedup+F-5+reversión end-to-end) · antes: **M2.3 CERRADO** — payment link colapsado)
...
**Tests**: 4.826 pytest (316 dbharness) + 363 vitest · ruff ≤196 · CI 5/5 · `certify_stg.sh` 18/18 · validate --ci 25/25.

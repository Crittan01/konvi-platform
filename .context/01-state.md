...
**Última actualización**: 2026-08-24 (**TRACK 5 M1 CERRADO + M2 DISEÑO PROPUESTO** — inventario verificable de capacidades por dominio (11 dominios, evidencia archivo:línea) + contrato de domain services propuesto (pilotos pedidos+reclamos) pendiente visto bueno founder · antes: **B-3 CERRADA** — harness de evaluación serio + Track 8 adversarial + E2E real de verificación + REORDEN founder: la plataforma primero, el BLOQUE BOT AL FINAL)
...
5. **EN CURSO — la plataforma primero**: Track 5 (M1-M5 dominios modulares) → Track 7 (UX/UI consola) → Track 3 (infra PRD) → remanentes Track 1/2 [F] → Track 4 ops. Orden completo y razón: `docs/PLAN-CIERRE.md` §Orden. **Track 5: M1 ✅ CERRADO 2026-08-24** (inventario verificable 11 dominios → `docs/architecture/domain-capabilities-inventory.md` — backlog exacto de la capa de servicios; drift medido: cancelación ×2, cliente Aveonline ×2, restock ×3, ingreso reconocido ×5, consent ×4) **+ M2 DISEÑO PROPUESTO, pendiente visto bueno founder** (`docs/architecture/domain-services-contract.md` — contrato `DomainCapability`, pilotos pedidos+reclamos, criterios de aceptación, **4 preguntas abiertas §8 que bloquean M2.1**: packaging shared-py, void automático desde consola, reason de reclamos, lecturas consola→REST). Sin código de producción hasta el visto bueno.

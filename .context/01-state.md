...
**Branch activo**: `develop`; `production` = `d8684b97` (deploy 2026-08-28 autorizado por founder — bundle código puro sin migraciones: B-2 Fase 0 + H11; los 3 servicios que no auto-dispararon se desplegaron vía Render API sobre el mismo commit; health ×5 verde con `api.konvi.co/health` reportando el SHA). Anterior: `d82d714c` (deploy 2026-08-27 — bundle: 7 migraciones aplicadas a PRD por protocolo + Track 5 M2 + Track 7 completo + Track 3 (dominios + pin 3.13.15 + PUBLIC_WEBHOOK_URL) + fix Aveonline RS256).

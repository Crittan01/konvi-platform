...
**Última actualización**: 2026-08-25 (**Track 7 EN CURSO — T7.1+T7.10+T7.5+T7.2+T7.3+T7.4+T7.6+T7.7 CERRADOS**: escena auth ×4 + logout de marca + topbar identidad + motion chat inbox dedupe-proof + pill bottom-nav viajera + LayoutItem pedidos + micro-celebraciones de dinero (dinero sandbox real) + barrido de estados — `RouteError` compartido con 23 boundaries por ruta, ~20 vacíos a `EmptyState`, skeletons a `ui/skeleton` + virtualización medida: NO paga en ninguna superficie (cotas verificadas live: audit 25/pág server-side, inbox cursor 50+50) — dep muerta `@tanstack/react-virtual` retirada, decisión + trigger en UX-UI §4.6. Fixes DS de las sondas live: `useReducedMotionDS` + `itemVariantsReduced` + Pressable tabindex SSR. Directiva founder 2026-08-25: la firma impregna TODO el front · antes: **M2.4 CERRADO — M2 COMPLETO**)
...
**Tests**: 4.826 pytest (316 dbharness) + 406 vitest · ruff ≤196 · CI 5/5 · `certify_stg.sh` 18/18 · validate --ci 25/25.
